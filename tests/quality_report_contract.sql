...
BEGIN
    ...
    INTO actual_count
    FROM data_quality_rule_report
    WHERE rule_id = 'deleted_user_with_addresses';

    IF actual_count <> 0 THEN
        RAISE EXCEPTION
            'Expected deleted users to have no related addresses, found %',
            actual_count;
    END IF;
    ...
    IF actual_count <> 0 THEN
        RAISE EXCEPTION
            'Expected every deleted user to include a deletion timestamp, found %',
            actual_count;
    END IF;
    ...
    IF actual_count <> 17 THEN
        RAISE EXCEPTION
            'Expected 17 actual issues across all severity summaries, found %',
            actual_count;
    END IF;
